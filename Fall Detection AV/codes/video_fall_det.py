...
import math                                    #for maths functions to calculate angles
...
import os                                      #for os access to save and remove files
import moviepy.editor as mpy                   #for extracting audio from an video file
...
#variables to set the number of images to be sent
...
#it extracts audio from a video file then runs speech recognition on that

def listen(video_file):
                    ...
                    if is_fallen(left_shoulder, left_hip, left_knee, left_ankle, right_shoulder, right_hip, right_knee, right_ankle):
                        if(length - breadth < 0):
                            if fall_count< 2:
                                ...
                                cv2.imwrite(f'fall_{fall_count+1}.jpg',image)
                                print("message sent")
                                send_image(f'fall_{fall_count+1}.jpg')
                                fall_count += 1
...
def run_audio_listener(video_file):
    while True:
        query = listen(video_file)
        if "help" in query:
...
if __name__ == "__main__":
    ...
    video_file = "address to video file for fall detection"
    video_thread = threading.Thread(target=fall_det, args=(cv2.VideoCapture(video_file),))
    video_thread.start()

    audio_thread = threading.Thread(target=run_audio_listener, args=(video_file,))
    audio_thread.start()
