        ...
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                if (landmarks[mp_pose.PoseLandmark.LEFT_HIP.value].y > landmarks[mp_pose.PoseLandmark.LEFT_KNEE.value].y and
                    ...
                    sit_flag = True
                    sit_counter += 1
                    if sit_counter >= sit_threshold:
                        frames_since_sit = 0
                else:
                    sit_flag = False
                    sit_counter = 0
                    
                # Increment frames since sit if not sitting
                if not sit_flag:
                    frames_since_sit += 1
                
                # Perform fall detection only if person has sat down recently
                if frames_since_sit <= sit_threshold:
                    # Continue with fall detection
                    # Extract landmarks...
                    pass
...
# def run_fall_detection(video_path):
#     fall_det(cv2.VideoCapture(video_path))
...
if __name__ == "__main__":
    ...
    video_capture  = cv2.VideoCapture("../fdx2/dataset/FALL-Forward_.mp4")
